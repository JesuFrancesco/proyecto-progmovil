...
import { ProductRatingFindFirst } from './ProductRatingFindFirst';
import { ProductRatingFindMany } from './ProductRatingFindMany';
import { ProductRatingFindUnique } from './ProductRatingFindUnique';
import { ProductRatingCreate } from './ProductRatingCreate';
import { ProductRatingCreateMany } from './ProductRatingCreateMany';
import { ProductRatingUpdate } from './ProductRatingUpdate';
import { ProductRatingUpdateMany } from './ProductRatingUpdateMany';
import { ProductRatingUpsert } from './ProductRatingUpsert';
import { ProductRatingDelete } from './ProductRatingDelete';
import { ProductRatingDeleteMany } from './ProductRatingDeleteMany';
import { ProductRatingAggregate } from './ProductRatingAggregate';
import { ProductRatingCount } from './ProductRatingCount';
import { ProductRatingGroupBy } from './ProductRatingGroupBy';
...
/**
 * Generates an Express router for ProductRating model.
 * @param config Contains optional middleware to enable routes.
 * @param customUrlPrefix Optional custom URL prefix for the routes.
 * @returns {express.Router}
 */
export function ProductRatingRouter(config: RouteConfig<RequestHandler>) {
  const router = express.Router();
  const basePath = removeTrailingSlash(config.customUrlPrefix || '') +
                  removeTrailingSlash(config.addModelPrefix !== false ? '/productrating' : '');
  ...
  if (config.enableAll || config?.findFirst) {
    ...
    setupRoute('/first', 'get', before, ProductRatingFindFirst as RequestHandler, inputValidator, outputValidator);
    if (after.length) {
  ...
  if (config.enableAll || config?.findMany) {
    ...
    setupRoute('/', 'get', before, ProductRatingFindMany as RequestHandler, inputValidator, outputValidator);
    if (after.length) {
  ...
  if (config.enableAll || config?.findUnique) {
    ...
    setupRoute('/:id', 'get', before, ProductRatingFindUnique as any, inputValidator, outputValidator);
    if (after.length) {
  ...
  if (config.enableAll || config?.create) {
    ...
    setupRoute('/', 'post', before, ProductRatingCreate as RequestHandler, inputValidator, outputValidator);
    if (after.length) {
      router.use(removeTrailingSlash(basePath + '/'), ...after);
    }
  }
  ...
  if (config.enableAll || config?.createMany) {
    ...
    setupRoute('/many', 'post', before, ProductRatingCreateMany as RequestHandler, inputValidator, outputValidator);
    if (after.length) {
  ...
  if (config.enableAll || config?.update) {
    ...
    setupRoute('/', 'put', before, ProductRatingUpdate as RequestHandler, inputValidator, outputValidator);
    if (after.length) {
      router.use(removeTrailingSlash(basePath + '/'), ...after);
    }
  }
  ...
  if (config.enableAll || config?.updateMany) {
    ...
    setupRoute('/many', 'put', before, ProductRatingUpdateMany as RequestHandler, inputValidator, outputValidator);
    if (after.length) {
      router.use(removeTrailingSlash(basePath) + '/many', ...after);
    }
  }
  ...
  if (config.enableAll || config?.upsert) {
    ...
    setupRoute('/', 'patch', before, ProductRatingUpsert as RequestHandler, inputValidator, outputValidator);
    if (after.length) {
      router.use(removeTrailingSlash(basePath + '/'), ...after);
    }
  }
  ...
  if (config.enableAll || config?.delete) {
    ...
    setupRoute('/', 'delete', before, ProductRatingDelete as RequestHandler, inputValidator, outputValidator);
    if (after.length) {
      router.use(removeTrailingSlash(basePath + '/'), ...after);
    }
  }
  ...
  if (config.enableAll || config?.deleteMany) {
    ...
    setupRoute('/many', 'delete', before, ProductRatingDeleteMany as RequestHandler, inputValidator, outputValidator);
    if (after.length) {
      router.use(removeTrailingSlash(basePath) + '/many', ...after);
    }
  }
  ...
  if (config.enableAll || config?.aggregate) {
    ...
    setupRoute('/aggregate', 'get', before, ProductRatingAggregate as RequestHandler, inputValidator, outputValidator);
    if (after.length) {
  ...
  if (config.enableAll || config?.count) {
    ...
    setupRoute('/count', 'get', before, ProductRatingCount as RequestHandler, inputValidator, outputValidator);
    if (after.length) {
  ...
  if (config.enableAll || config?.groupBy) {
    ...
    setupRoute('/groupby', 'get', before, ProductRatingGroupBy as RequestHandler, inputValidator, outputValidator);
    if (after.length) {
