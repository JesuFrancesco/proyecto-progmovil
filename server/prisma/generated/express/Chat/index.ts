...
import { ChatFindFirst } from './ChatFindFirst';
import { ChatFindMany } from './ChatFindMany';
import { ChatFindUnique } from './ChatFindUnique';
import { ChatCreate } from './ChatCreate';
import { ChatCreateMany } from './ChatCreateMany';
import { ChatUpdate } from './ChatUpdate';
import { ChatUpdateMany } from './ChatUpdateMany';
import { ChatUpsert } from './ChatUpsert';
import { ChatDelete } from './ChatDelete';
import { ChatDeleteMany } from './ChatDeleteMany';
import { ChatAggregate } from './ChatAggregate';
import { ChatCount } from './ChatCount';
import { ChatGroupBy } from './ChatGroupBy';
...
/**
 * Generates an Express router for Chat model.
 * @param config Contains optional middleware to enable routes.
 * @param customUrlPrefix Optional custom URL prefix for the routes.
 * @returns {express.Router}
 */
export function ChatRouter(config: RouteConfig<RequestHandler>) {
  const router = express.Router();
  const basePath = removeTrailingSlash(config.customUrlPrefix || '') +
                  removeTrailingSlash(config.addModelPrefix !== false ? '/chat' : '');
  ...
  if (config.enableAll || config?.findFirst) {
    ...
    setupRoute('/first', 'get', before, ChatFindFirst as RequestHandler, inputValidator, outputValidator);
    if (after.length) {
  ...
  if (config.enableAll || config?.findMany) {
    ...
    setupRoute('/', 'get', before, ChatFindMany as RequestHandler, inputValidator, outputValidator);
    if (after.length) {
  ...
  if (config.enableAll || config?.findUnique) {
    ...
    setupRoute('/:id', 'get', before, ChatFindUnique as any, inputValidator, outputValidator);
    if (after.length) {
  ...
  if (config.enableAll || config?.create) {
    ...
    setupRoute('/', 'post', before, ChatCreate as RequestHandler, inputValidator, outputValidator);
    if (after.length) {
      router.use(removeTrailingSlash(basePath + '/'), ...after);
    }
  }
  ...
  if (config.enableAll || config?.createMany) {
    ...
    setupRoute('/many', 'post', before, ChatCreateMany as RequestHandler, inputValidator, outputValidator);
    if (after.length) {
  ...
  if (config.enableAll || config?.update) {
    ...
    setupRoute('/', 'put', before, ChatUpdate as RequestHandler, inputValidator, outputValidator);
    if (after.length) {
      router.use(removeTrailingSlash(basePath + '/'), ...after);
    }
  }
  ...
  if (config.enableAll || config?.updateMany) {
    ...
    setupRoute('/many', 'put', before, ChatUpdateMany as RequestHandler, inputValidator, outputValidator);
    if (after.length) {
      router.use(removeTrailingSlash(basePath) + '/many', ...after);
    }
  }
  ...
  if (config.enableAll || config?.upsert) {
    ...
    setupRoute('/', 'patch', before, ChatUpsert as RequestHandler, inputValidator, outputValidator);
    if (after.length) {
      router.use(removeTrailingSlash(basePath + '/'), ...after);
    }
  }
  ...
  if (config.enableAll || config?.delete) {
    ...
    setupRoute('/', 'delete', before, ChatDelete as RequestHandler, inputValidator, outputValidator);
    if (after.length) {
      router.use(removeTrailingSlash(basePath + '/'), ...after);
    }
  }
  ...
  if (config.enableAll || config?.deleteMany) {
    ...
    setupRoute('/many', 'delete', before, ChatDeleteMany as RequestHandler, inputValidator, outputValidator);
    if (after.length) {
      router.use(removeTrailingSlash(basePath) + '/many', ...after);
    }
  }
  ...
  if (config.enableAll || config?.aggregate) {
    ...
    setupRoute('/aggregate', 'get', before, ChatAggregate as RequestHandler, inputValidator, outputValidator);
    if (after.length) {
  ...
  if (config.enableAll || config?.count) {
    ...
    setupRoute('/count', 'get', before, ChatCount as RequestHandler, inputValidator, outputValidator);
    if (after.length) {
  ...
  if (config.enableAll || config?.groupBy) {
    ...
    setupRoute('/groupby', 'get', before, ChatGroupBy as RequestHandler, inputValidator, outputValidator);
    if (after.length) {
