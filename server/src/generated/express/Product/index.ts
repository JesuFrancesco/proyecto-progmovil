...
import { ProductFindFirst } from './ProductFindFirst';
import { ProductFindMany } from './ProductFindMany';
import { ProductFindUnique } from './ProductFindUnique';
import { ProductCreate } from './ProductCreate';
import { ProductCreateMany } from './ProductCreateMany';
import { ProductUpdate } from './ProductUpdate';
import { ProductUpdateMany } from './ProductUpdateMany';
...
import { ProductGroupBy } from './ProductGroupBy';
...
/**
 * Generates an Express router for Product model.
 * @param config Contains optional middleware to enable routes.
 * @param customUrlPrefix Optional custom URL prefix for the routes.
 * @returns {express.Router}
 */
export function ProductRouter(config: RouteConfig<RequestHandler>) {
  const router = express.Router();
  const basePath = removeTrailingSlash(config.customUrlPrefix || '') +
                  removeTrailingSlash(config.addModelPrefix !== false ? '/product' : '');
  ...
  if (config.enableAll || config?.findFirst) {
    ...
    setupRoute('/first', 'get', before, ProductFindFirst as RequestHandler, inputValidator, outputValidator);
    if (after.length) {
  ...
  if (config.enableAll || config?.findMany) {
    ...
    setupRoute('/', 'get', before, ProductFindMany as RequestHandler, inputValidator, outputValidator);
    if (after.length) {
  ...
  if (config.enableAll || config?.createMany) {
    ...
    setupRoute('/many', 'post', before, ProductCreateMany as RequestHandler, inputValidator, outputValidator);
    if (after.length) {
  ...
  if (config.enableAll || config?.update) {
    ...
    setupRoute('/', 'put', before, ProductUpdate as RequestHandler, inputValidator, outputValidator);
    if (after.length) {
      router.use(removeTrailingSlash(basePath + '/'), ...after);
    }
  }
  ...
  if (config.enableAll || config?.updateMany) {
    ...
    setupRoute('/many', 'put', before, ProductUpdateMany as RequestHandler, inputValidator, outputValidator);
    if (after.length) {
      router.use(removeTrailingSlash(basePath) + '/many', ...after);
    }
  }
  ...
  if (config.enableAll || config?.upsert) {
    ...
    setupRoute('/', 'patch', before, ProductUpsert as RequestHandler, inputValidator, outputValidator);
    if (after.length) {
      router.use(removeTrailingSlash(basePath + '/'), ...after);
    }
  }
  ...
  if (config.enableAll || config?.delete) {
    ...
    setupRoute('/', 'delete', before, ProductDelete as RequestHandler, inputValidator, outputValidator);
    if (after.length) {
      router.use(removeTrailingSlash(basePath + '/'), ...after);
    }
  }
  ...
  if (config.enableAll || config?.deleteMany) {
    ...
    setupRoute('/many', 'delete', before, ProductDeleteMany as RequestHandler, inputValidator, outputValidator);
    if (after.length) {
      router.use(removeTrailingSlash(basePath) + '/many', ...after);
    }
  }
  ...
  if (config.enableAll || config?.aggregate) {
    ...
    setupRoute('/aggregate', 'get', before, ProductAggregate as RequestHandler, inputValidator, outputValidator);
    if (after.length) {
  ...
  if (config.enableAll || config?.count) {
    ...
    setupRoute('/count', 'get', before, ProductCount as RequestHandler, inputValidator, outputValidator);
    if (after.length) {
  ...
  if (config.enableAll || config?.groupBy) {
    ...
    setupRoute('/groupby', 'get', before, ProductGroupBy as RequestHandler, inputValidator, outputValidator);
    if (after.length) {
  ...
  if (config.enableAll || config?.findUnique) {
    ...
    setupRoute('/unique', 'get', before, ProductFindUnique as any, inputValidator, outputValidator);
    if (after.length) {
